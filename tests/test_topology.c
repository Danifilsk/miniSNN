...
static int check_network_lifecycle_safety(void)
{
    Network net;
    Network empty = {0};
    ...
    if (network_update(NULL) != -1)
        return fail("network_update accepted NULL network");

    if (network_update(&empty) != -1)
        return fail("network_update accepted an empty network");

    if (!network_init(&net, 3))
        return fail("network_init failed for lifecycle safety");

    if (!network_connect(&net, 0, 1, W))
    {
        network_destroy(&net);
        return fail("setup connection failed for lifecycle safety");
    }
    ...
    {
        return fail("network_destroy did not clear network pointers");
    }
    ...
    if (net.size != 0 ||
        ...
        net.delay_cursor != 0)
    {
        return fail("network_destroy did not reset network metadata");
    }

    if (network_update(&net) != -1)
        return fail("network_update accepted destroyed network");

    if (network_set_external_current(&net, 0, I_EXT))
        return fail("set_external_current accepted destroyed network");

    if (network_connect(&net, 0, 1, W))
        return fail("network_connect accepted destroyed network");

    if (!network_init(&net, 2))
        return fail("network_init failed after double destroy");

    if (network_update(&net) < 0)
    {
        network_destroy(&net);
        return fail("valid network failed to update after double destroy");
    }

    if (net.step != 1)
    {
        network_destroy(&net);
        return fail("valid network did not advance after lifecycle reset");
    }
...
int main(void)
{
    ...
    if (!check_network_lifecycle_safety())
        return 1;
