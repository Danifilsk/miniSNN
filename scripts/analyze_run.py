...
def write_report(run_path: Path, metrics: dict[str, object], context: dict[str, object], full_notes: list[str]) -> Path:
    ...
    lines = [
        ...
        "",
        "HOMEOSTASE E ESTABILIDADE",
        f"ativa: {value(metrics, 'homeostasis_enabled')}",
        f"origem das metricas: {value(metrics, 'homeostasis_metrics_source')}",
        f"taxa-alvo: {value(metrics, 'homeostasis_target_rate')}",
        f"taxa populacional final: {value(metrics, 'homeostasis_population_rate_final')}",
        f"erro absoluto medio: {value(metrics, 'homeostasis_rate_error_mean_absolute')}",
        f"threshold final medio: {value(metrics, 'homeostasis_threshold_final_mean')}",
        f"eventos de scaling: {value(metrics, 'homeostasis_scaling_events')}",
        f"ganho inibitorio final: {value(metrics, 'homeostasis_inhibitory_gain_final')}",
        "",
        "RECOMPENSA, PUNICAO E ELEGIBILIDADE",
        f"ativa: {value(metrics, 'reward_enabled')}",
        f"origem das metricas: {value(metrics, 'reward_metrics_source')}",
        f"eventos: {value(metrics, 'reward_event_count')}",
        f"eventos positivos: {value(metrics, 'reward_positive_event_count')}",
        f"eventos negativos: {value(metrics, 'reward_negative_event_count')}",
        f"reward acumulado aplicado: {value(metrics, 'reward_cumulative_applied')}",
        f"reward absoluto acumulado: {value(metrics, 'reward_cumulative_absolute')}",
        f"elegibilidade absoluta media final: {value(metrics, 'reward_eligibility_final_mean_absolute')}",
        f"conexoes modificadas: {value(metrics, 'reward_modified_connection_fraction')}",
        f"mudanca assinada de peso por reward: {value(metrics, 'reward_weight_total_signed_change')}",
        "O sinal modulador e externo; esta secao nao recalcula a regra R-STDP.",
        "",
        "12. Desempenho",
