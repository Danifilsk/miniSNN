...
#define MAX_WEIGHT_CASES 16
#define SUSTAINED_ACTIVITY_STEP 900
...
static void reset_result(FineSweepResult *result, double inhibitory_weight)
{
...
static int finalize_result(FineSweepResult *result)
{
    for (int i = 0; i < INPUT_SOURCE_COUNT; i++)
    {
...
static int run_fine_sweep_case(
    double inhibitory_weight,
    FineSweepResult *result)
{
    Network net;
...
static int write_csv_row(FILE *csv, const FineSweepResult *result)
{
    if (fprintf(
            csv,
            "%.2f,%d,%d,%d,%.2f,%d,%d,%d\n",
...
static void update_summary(
    const FineSweepResult *result,
    const FineSweepResult **max_activity,
    const FineSweepResult **min_activity,
    const FineSweepResult **first_below_100)
{
...
static void print_sustained_weights(
    const FineSweepResult *results,
    int result_count)
{
    int found = 0;

    printf("Pesos com atividade sustentada:");

    for (int i = 0; i < result_count; i++)
    {
        if (results[i].last_active_step >= SUSTAINED_ACTIVITY_STEP)
        {
            printf(" %.2f", results[i].inhibitory_weight);
            found = 1;
        }
    }
...
int main(void)
{
    FineSweepResult results[MAX_WEIGHT_CASES];
    int result_count = 0;
    const FineSweepResult *max_activity = NULL;
    const FineSweepResult *min_activity = NULL;
    const FineSweepResult *first_below_100 = NULL;
    FILE *csv = fopen("results/experiments/inhibition/inhibition_fine_sweep.csv", "w");

    if (csv == NULL)
    {
        printf("Erro: nao foi possivel criar results/experiments/inhibition/inhibition_fine_sweep.csv.\n");
        return 1;
    }
    ...
    printf("=== Varredura fina de inibicao ===\n");
    printf(
        "peso | spikes | pico | passos_ativos | "
        "ultimo_passo_ativo | neuronios_ativos\n");

    for (double weight = -350.0; weight >= -425.0; weight -= 5.0)
    {
        if (result_count >= MAX_WEIGHT_CASES)
        {
            printf("Erro: capacidade interna de resultados excedida.\n");
            fclose(csv);
            return 1;
        }

        if (!run_fine_sweep_case(weight, &results[result_count]))
        {
            fclose(csv);
            return 1;
        }

        if (!write_csv_row(csv, &results[result_count]))
        {
        ...
        printf("%6.2f | %6d | %4d | %14d | %18d | %16d\n",
               results[result_count].inhibitory_weight,
               results[result_count].total_spikes,
               results[result_count].peak_spikes,
               results[result_count].active_timesteps,
               results[result_count].last_active_step,
               results[result_count].active_neurons);
    ...
    if (result_count == 0 ||
        ...
        min_activity == NULL)
    {
        printf("Erro: nenhum resultado foi gerado.\n");
        return 1;
    }

    printf("\n");
    printf("Maior atividade: %.2f (%d spikes)\n",
           max_activity->inhibitory_weight,
           max_activity->total_spikes);
    printf("Menor atividade: %.2f (%d spikes)\n",
    ...
    print_sustained_weights(results, result_count);
